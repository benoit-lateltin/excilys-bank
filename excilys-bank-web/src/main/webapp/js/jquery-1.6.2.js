...
(function(window, undefined) {
					...
					function resolveFunc(i) {
						return function(value) {
							...
							if (!(--count)) {
								...
								deferred.resolveWith(deferred, sliceDeferred
										.call(args, 0));
							}
						};
					}
			...
			.extend({
				...
				data : function(elem, name, data, pvt /* Internal Use Only */) {
					...
					if (!cache[id]) {
						...
						// TODO: This is a hack for 1.5 ONLY. Avoids exposing
						// jQuery
					...
					// TODO: This is a hack for 1.5 ONLY. It will be removed in
					// 1.6. Users should
				...
				removeData : function(elem, name, pvt /* Internal Use Only */) {
					...
					// We destroyed the entire user cache at once because it's
					// faster than
					// iterating through each key, but we need to continue to
					// persist internal
					// data if it existed
					if (internalCache) {
						cache[id] = {};
						...
						// Otherwise, we need to eliminate the expando on the
						// node to avoid
	...
	function handleQueueMarkDefer(elem, type, src) {
		var deferDataKey = type + "defer", queueDataKey = type + "queue", markDataKey = type
				+ "mark", defer = jQuery.data(elem, deferDataKey, undefined,
				true);
		if (defer
	...
	jQuery.extend({
		...
		_mark : function(elem, type) {
			if (elem) {
				type = (type || "fx") + "mark";
				jQuery.data(elem, type, (jQuery.data(elem, type, undefined,
						true) || 0) + 1, true);
			}
		},
		...
		_unmark : function(force, elem, type) {
			...
			if (elem) {
				type = type || "fx";
				var key = type + "mark", count = force ? 0 : ((jQuery.data(
						elem, key, undefined, true) || 1) - 1);
				if (count) {
	...
	var rnamespaces = /\.(.*)$/, rformElems = /^(?:textarea|input|select)$/i, rperiod = /\./g, rspaces = / /g, rescape = /[^\w\s.|`]/g, fcleanup = function(
			nm) {
	...
	jQuery.event = {
			...
			if (!elem) {
				...
				jQuery
						.each(
								jQuery.cache,
								function() {
									// internalKey variable is just used to make
									// it easier to find
									// and potentially change this stuff later;
									// currently it just
		...
		handle : function(event) {
			...
			var handlers = ((jQuery._data(this, "events") || {})[event.type] || [])
					.slice(0), run_all = !event.exclusive && !event.namespace, args = Array.prototype.slice
					.call(arguments, 0);
			...
			for (j = 0; j < live.length; j++) {
				...
				if (close.selector === handleObj.selector
					...
					if (handleObj.preType === "mouseenter"
						...
						// Make sure not to accidentally match a child element
	...
	jQuery.fn
			.extend({
				...
				is : function(selector) {
					return !!selector
							&& (typeof selector === "string" ? jQuery.filter(
			...
			.extend({
				...
				ajax : function(url, options) {
					...
					function done(status, statusText, responses, headers) {
						...
						// Complete
						completeDeferred.resolveWith(callbackContext, [ jqXHR,
								statusText ]);
