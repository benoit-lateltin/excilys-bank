/*
 * https://github.com/vickychijwani/jquery-notify-osd
 */
(function($) {
	var notif_obj;
	var defaults = {
		...
		create : function(options) {
			...
			var Point = function(x, y) {
				return {
					...
					lies_inside : function(region) {
						return ((this.y > region.top)
								&& (this.y < region.bottom)
								&& (this.x > region.left) && (this.x < region.right));
					},
			...
			var Region = function(data) {
				return {
					...
					to_string : function() {
						return "t: " + this.top + " l: " + this.left + " h: "
			...
			if ($('.notify-osd').length == 0) {
				notif_obj = $(
						'<div class="notify-osd"><div><table><tr><td class="notify-osd-content">'
								+ opts.text + '</td></tr></table></div></div>')
						.css({
			...
			notif_obj
					.extend({
						...
						set_icon : function(src) {
							if (src != '')
								$(this).find('tr').prepend_or_replace(
										'<td class="notify-osd-icon"><img src="'
												+ src + '" /></td>',
										'.notify-osd-icon');
							else
						...
						set_dismissable : function(dismissable) {
							if (opts.dismissable) {
								$(this)
										.children('div')
										.append_or_replace(
												'<a href="#" class="notify-osd-dismiss" title="Dismiss">x</a>',
												'.notify-osd-dismiss');
								$(".notify-osd-dismiss").unbind('click').click(
										function() {
											notif_obj.dismiss();
										});
							} else {
						...
						show : function() {
							this.set_text(opts.text).set_icon(opts.icon)
									.set_dismissable(opts.dismissable);
							...
							$(this)
									.fadeIn(
											...
											function() {
												...
												this.timeout = (!opts.sticky && opts.timeout) ? setTimeout(
														function() {
															notif_obj.dismiss();
														}, opts.timeout * 1000)
														: null;
												...
												notif_obj
														.set_click_through(opts.click_through);
			...
			var mousemove = function(e) {
				...
				if (mouse.lies_inside(buffer)) {
					// find the minimum distance of the mouse from the edges of
					// the buffer region
					...
					if (mouse.lies_inside(notification))
						...
						opacity = opts.opacity_max
								- (opts.opacity_max - opts.opacity_min)
								* (min_distance / opts.buffer);
				} else {
					opacity = opts.opacity_max;
				}
